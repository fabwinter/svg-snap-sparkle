...
export const PRESETS: Record<PresetType, PresetDefaults> = {
  ...
  photo: { label: 'Photo', colorCount: 16, traceMode: 'spline', removeBg: false },
};
...
export function buildTraceConfig(preset: PresetType, colorCount: number): TraceConfig {
  switch (preset) {
    case 'logo':
      return {
        mode: 'outline', turdSize: 2, optTolerance: 0.1,
        colorPrecision: colorCount, pathOverlap: 0, filterSpeckle: 0, bwThreshold: 128,
      };
    case 'clipart':
    case 'illustration':
      return {
        mode: 'color', turdSize: 4, optTolerance: 0.2,
        colorPrecision: colorCount, pathOverlap: 4, filterSpeckle: 0,
      };
    case 'photo':
      return {
        mode: 'spline', colorPrecision: colorCount,
        filterSpeckle: 4, pathOverlap: 0, optTolerance: 0.2, turdSize: 2,
      };
  }
}
